...
class MakePayment:
    def __init__(self, username):
        ...
        if status == "E":
            utils.print_message("ATTENTION", "you have aleady paid your fee")

        # Checking if the student is just a candidate.
        elif status == "C":
            utils.print_message(
                "ATTENTION", "upload your documents first to pay your fee"
            )
        ...
        while True:
            ...
            while True:
                try:
                    ...
                    utils.print_message(
                        "CAUTION", 'invalid input. sample input: "1001, 1002, 1003"'
                    )
            ...
            if len(self.taken_courses) >= 3:
                ...
                utils.print_message("CAUTION", "you have to take atleast 3 courses.")
                print()
    ...
    def calculate_fee(self, taken_courses):
        ...
        print("\n" + utils.SPACING, "-" * 33)
        print(utils.SPACING, "RRC POLYTECH".center(33))
        print(utils.SPACING, "Fee Receipt".center(33))
        print(utils.SPACING, str(datetime.date.today()).center(33))
        print(
            utils.SPACING, str(datetime.datetime.now().strftime("%H:%M:%S")).center(33)
        )
        print(utils.SPACING, "-" * 33)
        print(utils.SPACING, "Description", "Amount".rjust(21))
        print(utils.SPACING, "-" * 33)
        ...
        for expence in amanities:
            print(
                utils.SPACING,
                expence[0],
                str("%.2f" % expence[1]).rjust(32 - len(expence[0])),
                "+",
            )
            total += expence[1]

        print(utils.SPACING, "-" * 33)
        print(utils.SPACING, "Total", str("%.2f" % total).rjust(27))
        print(utils.SPACING, "-" * 33)

        # Calculating the tax on the fee
        # GST
        GST = utils.get_tax("GST")
        gst = total * (GST / 100)
        print(utils.SPACING, f"GST({GST}%):", str("%.2f" % gst).rjust(24), "+")

        # PST
        PST = utils.get_tax("PST")
        pst = total * (PST / 100)
        print(utils.SPACING, f"PST({PST}%):", str("%.2f" % pst).rjust(24), "+")

        # Calculating Printing the sub total
        sub_total = total + gst + pst
        print(utils.SPACING, "-" * 33)
        print(utils.SPACING, "Sub Total:", str("%.2f" % sub_total).rjust(22))
        print(utils.SPACING, "-" * 33)
        ...
        if len(taken_courses) == 5:
            ...
            discount = sub_total * (utils.DISCOUNT / 100)
            print(
                utils.SPACING,
                f"Discount({utils.DISCOUNT}%):",
                str("%.2f" % discount).rjust(19),
                "-",
            )
            grand_total = sub_total - discount
        else:
            print(utils.SPACING, "Discount(0%):", "00.00".rjust(19), "-")

        # Calculating Printing the grand total
        print(utils.SPACING, "=" * 33)
        print(utils.SPACING, "Grand Total:", str("%.2f" % grand_total).rjust(20))
        print(utils.SPACING, "=" * 33)
    ...
    def make_payment(self, username):
        ...
        while True:
            ...
            if not len(card_num) == 16:
                utils.print_message("ERROR", "Please enter a valid card number")
                continue
            ...
            while True:
                try:
                    ...
                    utils.print_message("ERROR", "Please enter a valid date")
                    continue
            while True:
                cvv = input("3 digit security code: ")
                if not len(cvv) == 3:
                    utils.print_message("ERROR", "Enter Valid CVV")
                    continue
                break
        ...
        if "pay" in payment_method:
            ...
            utils.print_message(
                "CONGRATULATIONS", "you have paid your fee successfully"
            )

            try:
                # inserting course records
                utils.register_courses(username, self.taken_courses)
                # Changing the student status to approved.
                utils.set_status(username, "E")
                try:
                    mailing.send_offer_letter(username)
                    print(
                        "\n"
                        + "*an email has been sent to your registered email*".center(
                            utils.WIDTH
                        )
                    )
            ...
            utils.print_message("ERROR", "fee could not be paid")
    ...
    def fetch_courses(self, taken_courses=None):
        ...
        if taken_courses:
            ...
            query = (
                "SELECT course_id, course_name, course_fee FROM course WHERE active=1"
            )
            cur.execute(query)
